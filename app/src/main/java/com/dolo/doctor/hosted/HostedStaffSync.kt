...
object HostedPublicIdentityJson {
 fun parse(json:String,expectedRole:HostedStaffRole):HostedPublicIdentity{val root=JSONObject(json);require(root.optBoolean("authoritative")&&root.getString("privacy")=="SELF_ONLY_NO_PHONE"&&root.getString("productionEnrollment")=="DISABLED");val item=root.getJSONObject("identity");val role=HostedStaffRole.valueOf(item.getString("role"));val result=HostedPublicIdentity(item.getString("doloId"),item.getString("displayName"),role,item.getBoolean("prototype"));val expectedPrefix=if(role==HostedStaffRole.DOCTOR)"DLO-DOC-" else "DLO-AST-";require(role==expectedRole&&result.prototype&&result.doloId.matches(Regex("^${expectedPrefix}[0-9]{6}$"))&&result.displayName.isNotBlank()&&result.displayName.length<=120);return result}
}
