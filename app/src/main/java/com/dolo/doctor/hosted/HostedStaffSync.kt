...
class HttpHostedStaffApi(baseUrl:String,private val store:HostedStaffTokenStore,private val preferences:SharedPreferences){
 ...
 fun connectPilot(doloId:String,credential:String):HostedResult<PilotStaffIdentity> = guarded { val response=requestRaw("POST","/api/v1/auth/pilot/sessions",JSONObject().put("doloId",doloId.trim().uppercase()).put("credential",credential).put("deviceLabel","DO-LO Doctor Android").toString());val item=JSONObject(response).getJSONObject("identity");require(item.optBoolean("controlledPilot")&&!item.optBoolean("seededDummy")&&item.getString("role")=="DOCTOR");val identity=PilotStaffIdentity(item.getString("doloId"),item.getString("displayName"),HostedStaffRole.DOCTOR);require(identity.doloId.matches(Regex("^DLO-DOC-[0-9]{6}$")));store.save(parsePilotTokens(response,identity.role));identity }
 fun activatePilot(inviteCode:String,credential:String):HostedResult<PilotStaffIdentity> = guarded { val response=requestRaw("POST","/api/v1/auth/pilot/activate",JSONObject().put("inviteCode",inviteCode.trim()).put("expectedRole","DOCTOR").put("credential",credential).put("deviceLabel","DO-LO Doctor Android").toString());val item=JSONObject(response).getJSONObject("identity");require(item.optBoolean("controlledPilot")&&!item.optBoolean("seededDummy")&&item.getString("role")=="DOCTOR");val identity=PilotStaffIdentity(item.getString("doloId"),item.getString("displayName"),HostedStaffRole.DOCTOR);require(identity.doloId.matches(Regex("^DLO-DOC-[0-9]{6}$")));store.save(parsePilotTokens(response,identity.role));identity }
...
class HostedStaffViewModel(private val api:HttpHostedStaffApi):ViewModel(){
 ...
 private fun onboardingMessage(workspace:HostedDoctorOnboardingWorkspace):String=when{workspace.workspaceReady->"Doctor and clinic setup approved. Open the hosted clinic.";workspace.onboarding?.status==HostedDoctorOnboardingStatus.PENDING->"Setup submitted. Waiting for Admin review.";workspace.onboarding?.status==HostedDoctorOnboardingStatus.REJECTED->workspace.onboarding.reviewNote.ifBlank{"Admin requested corrections. Update and resubmit."};else->"Complete your Doctor and clinic setup for Admin review."} private fun executeSchedule(call:()->HostedResult<HostedClinicSchedule>){if(scheduleUiState.loading)return;scheduleUiState=scheduleUiState.copy(loading=true,error=false,message="Updating authoritative hosted schedule...");executor.execute{val r=call();main.post{scheduleUiState=when(r){is HostedResult.Success->HostedClinicScheduleUiState(schedule=r.value,message="Hosted schedule saved. Patient availability will refresh from the server.");is HostedResult.Failure->scheduleUiState.copy(loading=false,error=true,message=r.message)}}}}
