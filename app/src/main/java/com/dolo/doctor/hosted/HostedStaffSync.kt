...
class HttpHostedStaffApi(baseUrl:String,private val store:HostedStaffTokenStore,private val preferences:SharedPreferences){
 ...
 private fun requestNetwork(method:String,path:String,body:String?,bearer:String?,headers:Map<String,String>):String{
  val c=(URL(base+path).openConnection() as HttpURLConnection).apply{requestMethod=method;connectTimeout=15_000;readTimeout=25_000;setRequestProperty("Accept","application/json");setRequestProperty("User-Agent","DO-LO-Doctor-Android/Stage63PB");bearer?.let{setRequestProperty("Authorization","Bearer $it")};headers.forEach{(k,v)->setRequestProperty(k,v)};if(body!=null){doOutput=true;setRequestProperty("Content-Type","application/json")};useCaches=false}
...
class HostedStaffViewModel(private val api:HttpHostedStaffApi):ViewModel(){
 ...
 fun bindLocalRole(localRole:com.dolo.doctor.data.model.UserRole){val expected=HostedRoleBoundary.expected(localRole);val mismatch=api.savedRole()?.let{it!=expected}==true||uiState.snapshot?.let{it.role!=expected}==true;if(!mismatch)return;val token=api.detachSession();uiState=HostedStaffUiState(loading=true,message="Clearing a hosted session that belongs to another local role...");profileUiState=HostedDoctorProfileUiState();scheduleUiState=HostedClinicScheduleUiState();executor.execute{api.revokeDetached(token);api.detachSession();main.post{uiState=HostedStaffUiState(message="Previous hosted role cleared. Connect as ${expected.name.lowercase()}.")}}}
 fun localLogout(){val token=api.detachSession();uiState=HostedStaffUiState(message="Local account signed out; hosted session cleared.");profileUiState=HostedDoctorProfileUiState();scheduleUiState=HostedClinicScheduleUiState();executor.execute{api.revokeDetached(token);api.detachSession()}}
 ...
 fun logout(){if(uiState.loading)return;uiState=uiState.copy(loading=true,message="Clearing hosted session...");executor.execute{api.logout();main.post{uiState=HostedStaffUiState(message="Hosted session cleared. Local Doctor login is unchanged.");profileUiState=HostedDoctorProfileUiState();scheduleUiState=HostedClinicScheduleUiState()}}}
