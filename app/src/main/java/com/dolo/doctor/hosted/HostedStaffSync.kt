...
data class HostedStaffSnapshot(val role:HostedStaffRole,val permissions:Set<String>,val clinic:HostedClinic,val sessions:List<HostedSession>,val appointments:List<HostedAppointment>,val queues:List<HostedQueue>,val assistants:List<HostedAssistant>,val announcements:List<HostedAnnouncement> = emptyList(),val patientReviews:List<HostedPatientReview> = emptyList())
...
class HttpHostedStaffApi(baseUrl:String,private val store:HostedStaffTokenStore,private val preferences:SharedPreferences){
 ...
 private fun load():HostedStaffSnapshot{val snapshot=parseSnapshot(request("POST","/api/v1/staff/sync/bootstrap","{}"));return if(snapshot.role==HostedStaffRole.DOCTOR)snapshot.copy(announcements=HostedAnnouncementJson.parse(request("GET","/api/v1/staff/announcements")),patientReviews=HostedPatientReviewJson.parse(request("GET","/api/v1/staff/reviews"))) else snapshot}
