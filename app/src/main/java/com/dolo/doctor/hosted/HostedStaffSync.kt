...
class HttpHostedStaffApi(baseUrl:String,private val store:HostedStaffTokenStore,private val preferences:SharedPreferences){
 ...
 private fun load():HostedStaffSnapshot{
  ...
  return if(snapshot.role==HostedStaffRole.DOCTOR)snapshot.copy(announcements=HostedAnnouncementJson.parse(request("GET","/api/v1/staff/announcements")),patientReviews=HostedPatientReviewJson.parse(request("GET","/api/v1/staff/reviews"))) else snapshot
 }
