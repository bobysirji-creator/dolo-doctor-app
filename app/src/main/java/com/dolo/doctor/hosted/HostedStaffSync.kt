...
class HostedStaffViewModel(private val api:HttpHostedStaffApi):ViewModel(){
 ...
 fun logout(){if(uiState.loading)return;uiState=uiState.copy(loading=true,message="Clearing hosted session...");executor.execute{api.logout();main.post{uiState=HostedStaffUiState(message="Hosted session cleared. Local Doctor login is unchanged.");profileUiState=HostedDoctorProfileUiState()}}}
