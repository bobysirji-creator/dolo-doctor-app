...
Queue progress is now monotonic. `Last token` is a public progress marker and is updated with `max(previous progress, called token)`. An active consultation is identified by appointment status rather than equality with the progress token, so a late token 6 served after token 15 cannot move the display or Patient tracking backward. The queue list still identifies token 6 as the active Patient. Existing persistence remains compatible. GitHub Actions run 31809548353 passed compile, lint, unit tests, signing verification and stable APK generation. Install that artifact over the accepted Doctor App and follow `docs/stage62g-device-test.md`.
## Stage 62H implementation checkpoint

Doctor App `0.30.0-stage62h` (version code 43) restores a compact shared DO-LO Doctor logo throughout the app, enlarges the Home profile-image placeholder and replaces the separate More destination with an overlay navigation drawer. The drawer retains the existing Doctor/Assistant permission policy and contains theme and logout controls.

Notifications now show only authoritative hosted notification records. Local queue, appointment, consultation-fee, receipt, session, availability and configuration audit events remain in Activity log and no longer inflate the bell badge or Notifications screen. Logout clears the complete authenticated navigation graph, and Android Back exits from Home while continuing to navigate normally on child pages. Existing queue progress, local data and hosted data contracts remain unchanged. GitHub Actions run 31823774406 passed compilation, lint, unit tests, signing verification and stable APK generation. Install the stable artifact over the accepted app and follow `docs/stage62h-device-test.md`.